...
function onSubmit(e) {
	...
	var queryURL =
		"https://app.ticketmaster.com/discovery/v2/events.json?countryCode=US&city=Chicago&classificationName=music&classificationName=" +
		genre +
		"&startDateTime=" +
		startDate +
		"&endDateTime=" +
	...
	$.ajax({
		...
		if (response.page.totalElements > 0) {
			...
			for (var i = 0; i < events.length; i++) {
				...
				var myButton = $(
					"<button class='api-btn'>" + events[i].name + "</button>"
				);
			...
			$(".error-msg").text(
				"Your search came up empty. Try another date range or genre."
			);
		}
	});
}
...
function changeSrc(myobj) {
	...
	//change the path of the iframe to what the user has typed
	$("#myFrame").attr(
		"src",
		"https://www.google.com/maps/embed/v1/search?q=" +
			userLocation +
			"&key=AIzaSyB7ydrZE1U4_y3TjyeaO2aVyfWzxUnxKuk"
	);
}
